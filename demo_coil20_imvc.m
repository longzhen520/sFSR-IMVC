...
%      A=[0.02,0.03,0.04,0.05,0.1];
   %% yale   j=8;k=1e-4;a=0.1;
   %% coil 
   j=4;k=1e-3;a=0.05;
   %% BDGP j=6;k=1e-3;a=0.03; 
   %% Scence15   j=4;k=1e-3;a=0.002
   %% CCV      j=4;k=1;a=0.002
   %% Calteh-all j=4;k=1e-3;a=0.002
   %% Animal   j=6;k=1e-2;a=0.002
   %% AOLI    j=6;k=1e-2;a=0.002
   %% Reuters j=2;k=1e-3;a=0.002
%            for j=1:length(beta)
...
%      C = SpectralClustering(S,cls_num);% C = kmeans(U,numClust,'EmptyAction','drop'); 
 ...
 [max_ACC,id] = max(ACC(:));
  [max_ACC1,id1] = max(ACC1(:));
  if max_ACC>max_ACC1
Clu_result.ACC = max_ACC;
Clu_result.NMI = NMI(id);
Clu_result.Purity = Purity(id);
 else
 Clu_result.ACC = max_ACC1;
Clu_result.NMI = NMI1(id1);
Clu_result.Purity = Purity1(id1);
